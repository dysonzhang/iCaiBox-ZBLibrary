...
public class BluetoothLeService extends Service {
	private final static String TAG = "BLE_GATT"; // BluetoothLeService.class.getSimpleName();
	...
	public final static String ACTION_DATA_WRITEOVER = "com.example.bluetooth.le.ACTION_DATA_WRITEOVER";
	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
		...
		public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
			if (status == BluetoothGatt.GATT_SUCCESS) {
				broadcastReadUpdate(ACTION_DATA_READBACK, characteristic);
			}
		}
		...
		public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
			Log.i("GattTag", "notify have data");
	...
	private void broadcastReadUpdate(final String action, final BluetoothGattCharacteristic characteristic) {
		final Intent intent = new Intent(action);
		...
		// For all other profiles, writes the data formatted in HEX.
		final byte[] data = characteristic.getValue();
		Log.i("GattTag", "read, len= " + data.length);
		if (data != null && data.length > 0) {
			intent.putExtra(EXTRA_DATA, data);
		}
	...
	/// add by GattTag for write characteristic
