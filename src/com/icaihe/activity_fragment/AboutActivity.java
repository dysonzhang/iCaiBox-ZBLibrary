...
public class AboutActivity extends BaseActivity implements OnClickListener, OnLongClickListener, OnBottomDragListener {
	...
	public void initData() {
		...
		tvAboutAppInfo.setText(
				ICHApplication.getInstance().getAppName() + "\n" + ICHApplication.getInstance().getAppVersion());
		...
		runThread(TAG + "setQRCode", new Runnable() {
			...
			public void run() {
				...
				try {
					...
					e.printStackTrace();
					Log.e(TAG,
							"initData  try {Bitmap qrcode = EncodingHandler.createQRCode(contactJson, ivContactQRCodeCode.getWidth());"
