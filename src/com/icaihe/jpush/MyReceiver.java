...
public class MyReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
			...
			strContentType = bundle.getString(JPushInterface.EXTRA_CONTENT_TYPE);
			...
			Notification.Builder builder = new Notification.Builder(context).setAutoCancel(true)
					.setDefaults(Notification.DEFAULT_VIBRATE).setSmallIcon(R.drawable.icon)
			...
			PendingIntent pdmsg = PendingIntent.getBroadcast(context, 0, intentMsg, PendingIntent.FLAG_UPDATE_CURRENT);

			builder.setContentTitle(strTitle).setContentText(strMessage).setContentIntent(pdmsg);
		...
		} else if (intent.getAction().equals(com.icaihe.jpush.MyReceiver.ICAIHE_NOTIFICATION_CLICK)) {
			...
			processNotifaction(context, bundle);
	...
	private void processNotifaction(Context context, Bundle bundle) {
		// BaseActivity推送通知
		// if (BaseActivity.isForeground) {
		String extras = bundle.getString(JPushInterface.EXTRA_EXTRA);
		Intent msgIntent = new Intent(BaseActivity.NOTIFACYTION_RECEIVED_ACTION);
